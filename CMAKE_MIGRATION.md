...
│   ├── linux.x86/          # Linux x86 target
│   ├── mach.x86_64/        # macOS target
...
The following target architectures are defined:

- **linux.x86** - Linux 32-bit x86 (default)
- **linux.llvm** - Linux with LLVM/Clang
- **linux.armv7a** - Linux ARM v7-A
- **mach.x86_64** - macOS 64-bit
- **freertos.armv7m** - FreeRTOS ARM v7-M
- **freertos.armv7m.mbed-lpc1768** - FreeRTOS ARM v7-M with mbed LPC1768
- **freertos.armv6m** - FreeRTOS ARM v6-M
- **freertos.armv4t** - FreeRTOS ARM v4-T
- **freertos.mips4k.pic32mx** - FreeRTOS MIPS PIC32MX
- **bare.armv7m** - Bare metal ARM v7-M
- **js.emscripten** - Emscripten JavaScript
- **mingw.x86** - MinGW 32-bit Windows
- **nonos.xtensa-lx106.esp8266** - ESP8266 (non-OS)
...
```bash
# Configure for default target (linux.x86)
cmake -B build
...
- Currently, not all target architectures have CMakeLists.txt files created
